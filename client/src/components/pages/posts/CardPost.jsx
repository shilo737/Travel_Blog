...
const CardPost = ({ post, toggleFavorite, setRefreshDeletePost ,user}) => {
    ...
    <div className="containers">
      <div className="cards rounded-2xl">
        ...
        <div className="absolute flex gap-2 right-2 bottom-2 text-indigo-800 text-[2em] cursor-pointer items-center">
          {user?.role === "admin" && (
            <div className="text-red-500" onClick={() => deleteMyPost(_id)}>
              <svg
          ...
          {user && user._id && (
            <div
              onClick={() =>
                toggleFavorite(_id, () => setIsFavorite((prev) => !prev))
              }
            >
              {isFavorite ? (
                <FaHeart color="red" />
